...
@SpringBootTest
@PactTestFor(providerName = "pactflow-example-consumer-java-kafka")
public class ConsumerTest {

    @Pact(consumer = "pactflow-example-provider-java-kafka")
    V4Pact createPactAsync(MessagePactBuilder builder) {
        PactDslJsonBody body = new PactDslJsonBody();
        body.stringType("name", "123abcd");
...
//        })
    ...
    void test(V4Interaction.AsynchronousMessage message) throws Exception {
        ...
        assertDoesNotThrow(() -> {
            MyMessageHandler.getRequest(response);
        });
    }
